...
public:
  ...
  getLocationsFromURL(const QUrl &url, const std::tuple<QString, QString> &customLocation = defaultLocation) const {
    ...
    if (url.scheme().startsWith("http")) {
      JsonFetcher fetcher;
      json = fetcher.getJsonFromURL(url.toString());
    } else {
      QString localFilePath = url.toString();
      qDebug() << "Loading local file:" << localFilePath;
      ...
      if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        qWarning() << "Failed to open local file:" << localFilePath;
        return locations;
      }
    }
  ...
  getOsmLocations(const std::tuple<QString, QString> &customLocation = defaultLocation) const {
    return getLocationsFromURL("../assets/nation_bounding_boxes.json", customLocation);//"https://raw.githubusercontent.com/pfeiferj/openpilot-mapd/main/nation_bounding_boxes.json", customLocation);
  }

  inline std::vector<std::tuple<QString, QString, QString, QString>>
  getUsStatesLocations(const std::tuple<QString, QString> &customLocation = defaultLocation) const {
    return getLocationsFromURL("../assets/us_states_bounding_boxes.json", customLocation);//"https://raw.githubusercontent.com/pfeiferj/openpilot-mapd/main/us_states_bounding_boxes.json", customLocation);
  }
};
